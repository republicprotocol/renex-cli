...
async function main() {
    if (process.argv[2] === 'init' && process.argv.length == 4) {
        var password1 = "";
        var password2 = " ";
        ...
        return
    } else if (process.argv[2] === 'get-balances' && process.argv.length == 4) {
    ...
    } else if (process.argv[2] === 'open-order' && process.argv.length == 3) {
        const password = await promptly.password(chalk.bold.cyan('Enter your password: '));
        const wallet = Wallet.fromV3(fs.readFileSync('encrypted_keystore.json').toString(), password, true);
